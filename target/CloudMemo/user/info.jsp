  ...
  <div class="data_list">
    ...
    <div class="container-fluid">
      <div class="row" style="padding-top: 20px;">
        ...
        <div class="col-md-4"><img style="width:200px;height:200px" src="user?actionName=userHead&imageName=${user.head}"></div>
      </div>
    </div>
  </div>
</div>
<script type="text/javascript">
  ...
  $("#nickName").blur(function (){
    var nickName = $("#nickName").val();//获取昵称文本的值
    if(isEmpty(nickName)){//判断是否为空
      $("#msg").html("用户名称不能为空!")
      $("#btn").prop("disabled",true);
      return;
    }
    var nick = '${user.nick}';//从session域中获取用户昵称
    if(nickName == nick){//如果用户昵称与session中的昵称一致,则return
      return;
    }
    $.ajax({
      type:"get",
  ...
  function updateUser() {
    var nickName = $("#nickName").val();//获取昵称文本的值
    if (isEmpty(nickName)) {//判断是否为空,为空,禁用按钮,return
      $("#msg").html("用户名称不能为空!")
      $("#btn").prop("disabled", true);
      return false;
     }
